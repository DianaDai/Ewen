...
class questioncontrol extends base {
 ...
 function onajaxanswer() {
 	...
 	if(!strstr($useragent, 'MicroMessenger')&&isset($this->setting['code_ask'])&&$this->setting['code_ask']=='1'&&$this->user['credit1']<$this->setting['jingyan']){
         ...
         if($hidanswer==1){
          ...
          $message['message']="问题包含非法关键词，发布失败!";
           ...
           if($this->user['groupid']==1){
            ...
            $qid = $_ENV['question']->add($title, $description, $hidanswer, $price, $cid, $cid1, $cid2, $cid3, $status,0,$askfromuid);
