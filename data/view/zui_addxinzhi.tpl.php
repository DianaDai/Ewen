...
<div class="user-home bg-white" >
    ...
    <div class="row">
        <div class="col-md-8">
        <input type="text" AUTOCOMPLETE="OFF" id ="title" name="title" value="<?=$topic['title']?>" size="50" class="form-control" placeholder="标题不能为空">
  ...
  <a class="text-danger" href="###" text="网页提问浮层添加图片点击">点击选择封面图片，大小建议288px*192px.如果不传就填外部图片地址也行
 
  </a><br>
    <img class="img-thumbnail" data-toggle="lightbox"  data-image="<?=SITE_URL?>static/css/dist/css/images/default.jpg" data-caption="图片浏览"  id="imghead" width=288 height=192 border=0 src='<?=SITE_URL?>static/css/dist/css/images/default.jpg'>
</div>
...
<br>
   <input type="file" id="layer_upload" onchange="previewImage(this)" name="image" title="请选择图片" accept="image/*" style="opacity: 1;height: 40px;position:relative;">
  </div>
  </div>
     ...
     <input type="text" AUTOCOMPLETE="OFF" id="outimgurl" name="outimgurl" value="" size="350" class="form-control" placeholder="如果不上传可以选择外部封面图片地址">
        ...
        <? if($user['grouptype']!=1&&$user['credit1']<$setting['jingyan']) { ?>                  <div class="form-group">
          ...
          <div class="col-md-6">
           <span  id="codetip" class="alert alert-warning ">验证码不能为空</span>
...
var fv1=$("#outimgurl").val();
var title=$("#title").val();
//var title =$("[name='']").val();
//var imageurl=$("[name='outimgurl']").val();
 if(v==''){
 alert("请选择文章分类");
