...
<form action="index.php?admin_setting/register<?=$setting['seo_suffix']?>" method="post">
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td class="altbg2">
                <input class="radio inline"  type="radio"  <? if(1==$setting['allow_register'] ) { ?>checked<? } ?>  value="1" name="allow_register"><label for="yes">是</label>&nbsp;&nbsp;&nbsp;&nbsp;
                <input class="radio inline"  type="radio"  <? if(0==$setting['allow_register'] ) { ?>checked<? } ?> value="0" name="allow_register"><label for="no">否</label></td>
        </tr>
         <tr>
            <td class="altbg1" width="45%"><b>注册用户必须邮箱验证:</b><br><span class="smalltxt">开启后没有邮箱验证的用户除了管理员外都不能进行一切操作</span></td>
            <td class="altbg2">
                <input class="radio inline"  type="radio"  <? if(1==$setting['register_on'] ) { ?>checked<? } ?>  value="1" name="register_on"><label for="yes">是</label>&nbsp;&nbsp;&nbsp;&nbsp;
                <input class="radio inline"  type="radio"  <? if(0==$setting['register_on'] ) { ?>checked<? } ?> value="0" name="register_on"><label for="no">否</label></td>
        </tr>
        <tr>
