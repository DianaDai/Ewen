...
<div class="ask_detail_item clearfix">
                ...
                <div class="ask_detail_content">
                       ...
                       <? if($question['hidden']==1) { ?>              <a class="avatar"  data-bn-ipg="7-1">
                            <img   src="<?=$question['author_avartar']?>" width="80" height="80" alt="作者已经匿名" >
                            
                        </a>
           <? } else { ?>          <a class="avatar" href="<?=SITE_URL?>u-<?=$question['authorid']?>.html" data-bn-ipg="7-1">
...
<? } } } else { } ?>                                        </div>
                    ...
                    <div class="question-info clearfix mt10">
                        ...
                        <?=$question['format_time']?>
                                                    ...
                                                    <? if($question['hidden']==1) { ?>                                                          来自 匿名网友  的问题
                                                    <? } else { ?>                                                          来自 <a title="<?=$question['author']?>" href="<?=SITE_URL?>u-<?=$question['authorid']?>.html" data-bn-ipg="7-4"><?=$question['author']?></a> 的问题
                                                    <? } ?>                            </span>
                              <? if($question['price']>0) { ?>                            <label class="icon_hot">悬赏<?=$question['price']?>金币</label>
                                      <? } ?>                                        <? if($question['askuid']>0) { ?>                            <label class="icon_zise">邀请<?=$question['askuser']['username']?>回答</label>
                                      <? } ?>                    </div>
                    <div class="ask_detail_content_text qyer_spam_text_filter">
                   <? echo replacewords($question['description']);     ?>                   <? if($supplylist) { ?>                       <ul class="nav">
...
<? } } ?>
  ...
  <? if($is_followed) { ?>   <a  href="javascript:;" id="attenttoquestion" value="2892089" class="button_followed ui_btn_big jsaddquestionatten" onclick="attentto_question(<?=$question['id']?>)" id="attenttoquestion" data-bn-ipg="ask-answer-follow">取消关注</a>

                    <? } else { ?>   <a href="javascript:;"  value="2892089" class="button_attention ui_btn_big jsaddquestionatten" onclick="attentto_question(<?=$question['id']?>)" id="attenttoquestion" data-bn-ipg="ask-answer-follow">关注问题</a>
  ...
  <div class="share-content" style="margin-left:25px;"><?=$setting['question_share']?></div>
   ...
   <? if($bestanswer['id']>0) { ?>   <div class="mod_discuss clearfix jsanswerbox" id='<?=$bestanswer['id']?>'>
    ...
    <div class="mod_discuss_cnt">
        ...
        <img src="<?=SITE_URL?>static/css/dist/css/images/zuijia.png" style="position:absolute;top:-40px;right:-30px;">
...
<? if(is_array($bestanswer['appends'])) { foreach($bestanswer['appends'] as $append) { ?>
                    <div class="appendbox">
                        ...
                        <? } ?>                              <div class="zhuiwentext">
                         <? echo replacewords($append['content']);     ?>                                        </div>
                    <div class="clr"></div>
                    </div>
                    ...
                    <div class="mt5">
                        ...
                        <i class="icon icon-window-alt mar-y-05 font12"></i>
                               ...
                               添加讨论(<?=$bestanswer['comments']?>)</span>
                                                  ...
                                                  <? if($user['grouptype']==1) { ?>                             <span class="span-line mar-ly-05">|</span>
    ...
    <div class="mod_discuss_cnt">
        ...
        <div class="mod_discuss_box">
            <div class="jsanswercontent">
                ...
                <div class="mod_discuss_box_text qyer_spam_text_filter">
                           <? echo replacewords($answer['content']);     ?>                 <div class="appendcontent">
...
<? } } ?>
      ...
      <? if($user['uid']>0) { ?>        <? if($bestanswer['id']<=0) { ?>         <? if(1==$user['grouptype'] ||$user['uid']==$question['authorid']) { ?>                                <a class="text-danger mar-l-05" data-placement="bottom" title="" data-toggle="tooltip" data-original-title="采纳回答" href="javascript:void(0);" onclick="adoptanswer(<?=$answer['id']?>);" >采纳为满意回答</a>
                                <? } ?>                             <? } ?>                          <? if($user['uid']==$question['authorid']) { ?>            <a data-placement="bottom" title="" data-toggle="tooltip" data-original-title="继续回答问题" href="<?=SITE_URL?>answer/append/<?=$question['id']?>/<?=$answer['id']?>.html" class="text-green mar-l-1 operate jsjubaoanswer mr10 fl"  > 继续追问</a>

              <? } elseif($user['uid']==$answer['authorid']) { ?>            <a data-placement="bottom" title="" data-toggle="tooltip" data-original-title="修改自己答案" class="mar-l-1 text-green" href="<?=SITE_URL?>question/editanswer/<?=$answer['id']?>.html">修改答案</a>
...
<? } } ?>
 <div class="pages" style="padding-left:30px;"><?=$departstr?></div>
                                    ...
                                    <? if(9!=$question['status']  ) { ?>                                                  <? if(0!=$user['uid']) { ?>   <? if(!$already && $user['uid']!= $question['authorid']) { ?>   <? if($setting['register_on']==1&&$user['active']!=1) { ?>    <p class="mar-t-1">由于网站设置，需要设置邮箱并且激活邮箱才能提问,回答，发布文章等一系列操作,<a class="text-danger" href="<?=SITE_URL?>user/editemail.html"> 激活邮箱走起!</a></p>
       <? } else { ?>                                <div class="mod_discuss clearfix mt10">
    ...
    <div class="col-sm-12 mar-t-05">
        ...
        <div class="row">
            <div class="col-sm-10">
              ...
              <? if($setting['code_ask']&&$user['credit1']<$setting['jingyan']) { ?>                <div class="row">
                    <div class="col-sm-4">
                        <span class="">验证码</span>
                        <input autocomplete="OFF"  onblur="check_code()" name="code" id="code" type="text" class="input-code">
                    </div>
            ...
            <div class="col-sm-2">
             <input type="hidden" value="<?=$question['id']?>" id="ans_qid" name="qid">
                ...
                $("#answsubmit").click(function(){
                  ...
                  <? } ?>                       <? } ?>   <? } else { ?>  <p class="text-left " style="padding-left:25px;"> <a class="text-danger" href="<?=SITE_URL?>user/login.html">登录</a>
    <!-- 或者   <a class="text-danger" href="<?=SITE_URL?>user/login.html">注册</a>  -->
    后回答问题
     <? } ?>      <? } else { ?>       <p class="text-center" style="padding-left:25px;"> 该问题目前已经被关闭, 无法添加新回复
       <? } ?></p>
