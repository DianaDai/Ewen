...
<div style="width:100%; height:15px;color:#000;margin:0px 0px 10px;">
    ...
    <table class="table">
        <tbody>
            ...
            <tr class="altbg1"><td colspan="3" class="text-danger ">1、用户名必须是系统已注册用户。2、分类为系统已添加分类，多个分类用,隔开,最多不超过3个</td></tr>
            <tr>
                <td width="30%" >用户名:<input  class="form-control shortinput inline" type="text" name="username"/></td>
                <td  width="40%">擅长分类ID:<input class="form-control shortinput inline" type="text" name="goodatcategory" id="categorys" size="50" onfocus="showselect()"/></td>
...
<? if(is_array($expert['category'])) { foreach($expert['category'] as $category) { ?>
                <a href="<?=SITE_URL?>c-<?=$category['cid']?>.html" class="label"  target="_blank"><?=$category['categoryname']?></a>&nbsp;&nbsp;
...
<? } } ?>
        ...
        <? } ?>        <tr class="altbg1"><td colspan="3" class="altbg1"  align="left"><input type="submit" name="submit" class="btn btn-info" value="提&nbsp;交" /></td></tr>
    </table>    
</form>
      ...
      <div id="dialogcate">
            ...
            <table class="table table-striped">
                ...
                <tr>
                    <td colspan="5">
                    <span>
                    <input  type="button" class="btn btn-info" value="确&nbsp;认" onclick="add_category();"/></span>
            ...
            <input type="button" class="btn btn-info" value="确认添加" onclick="save_change();"/>
        </div>
                    </td>
