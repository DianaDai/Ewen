...
<html lang="zh-cn"><? global $starttime,$querynum;$mtime = explode(' ', microtime());$runtime=number_format($mtime['1'] + $mtime['0'] - $starttime,6); $setting=$this->setting;$user=$this->user;$headernavlist=$this->nav;$regular=$this->regular;$toolbars="'".str_replace(",", "','", $setting['editor_toolbars'])."'"; ?><head>
    <meta charset="utf-8">
    ...
    <link href="<?=SITE_URL?>static/css/dist/css/user.css" rel="stylesheet">
    <? if(isset($setting['tpl_themedir'])&&$setting['tpl_themedir']!='default') { ?>    <link href="<?=SITE_URL?>static/theme/<?=$setting['tpl_themedir']?>/<?=$setting['tpl_themedir']?>.css" rel="stylesheet">
    <? } ?>    <!-- jQuery -->
...
<? if(is_array($headernavlist)) { foreach($headernavlist as $nav1) { ?>
                    <? if($nav1['type']==2 && $nav1['available']) { ?>                    <a  title="<?=$nav1['title']?>" href="<?=$nav1['url']?>"  <? if($nav1['target']) { ?>target="_blank"<? } ?>><?=$nav1['name']?></a>
...
<? } } ?>
            ...
            <div class="col-sm-4">
                ...
                <? if(0!=$user['uid']) { ?>                <div class="b_p2">
                    ...
                    <span class="b_span6 color-white" id="topSpan2"><a class="color-white" href="<?=SITE_URL?>message/system.html">消息<span class="msg-count icon_hot"></span></a></span>
...
<a class="dropdown-toggle color-white " data-toggle="dropdown" href="javascript:void(0);">管理中心<i class="icon icon-caret-down"></i></a>
 ...
 <ul class="dropdown-menu pull-right">
     ...
     <li><a href="<?=SITE_URL?>user/logout.html" ><i class="icon icon-signout text-success mar-ly-05"></i>退出网站</a></li>
                ...
                <? } else { ?>                <div class="b_p2">
                    <span class="b_span3"></span><span class="b_span4" id="topSpan1"><a href="<?=SITE_URL?>user/login.html">登录</a></span><span class="b_span5"></span>
...
<div class="search-part bg-white hidden-sm hidden-xs">
    <div class="container  ">
        <div class="row">
            <div class="col-md-5">
                <a class="navbar-brand bounceIn animated pull-left" style="padding: 10px 0px;padding-right: 15px;margin-left: -5px;" href="<?=SITE_URL?>">
                    <? if($setting['site_logo'] ) { ?>                    <!--<img src="<?=$setting['site_logo']?>" style="height: 90px;">-->
                    <img src="<?=SITE_URL?>data/attach/logo/logo.png" style="height: 90px;">
                    <? } else { ?>                    <img src="<?=SITE_URL?>static/css/soso/logo.png" style="height: 90px;">
                    <? } ?>                </a>
                <p class="pull-left" style="margin-top: 40px;margin-left: 30px;font-size: 40px;font-weight: bold;color: #ea644a;font-family: 'Microsoft YaHei'">

                    <? if(isset($setting['title_description']) ) { ?><?=$setting['title_description']?>
                    <? } else { ?>                    知名专家为您解答
                    <? } ?>                </p>
            </div>
            <div class="col-md-6" style="width: 46.3%;">
            ...
            <div class="col-md-1" style="float: right;width: 12%;">
                <a class="navbar-brand bounceIn animated" style="width: 200px;">
                    <img src="<?=SITE_URL?>data/attach/banner/Ewen.png" style="height: 80px;">
                    <div style="font-size: 15px;color: #ea644a;margin-left: 10px;">
                        扫码提问
                    </div>
                </a>
            </div>
...
<? if(is_array($headernavlist)) { foreach($headernavlist as $headernav) { ?>
            <? if($headernav['type']==1 && $headernav['available']) { ?>            <li <? if(strstr($headernav['url'],$regular)) { ?> class="nav2_current"<? } ?>>
            <a title="<?=$headernav['title']?>" href="<?=$headernav['format_url']?>"><?=$headernav['name']?></a>
            </li>

            <? } ?>            
<? } } ?>
            <? if(0!=$user['uid']) { ?>            <li style="float: right;cursor: pointer;" >
                <a title="发表文章" href="<?=SITE_URL?>user/addxinzhi.html">发表文章</a></li>
            <li style="float: right;cursor: pointer;">
                <a title="我要提问" onclick="window.location.href='<?=SITE_URL?>question/add.html'">我要提问</a></li>

            <? } else { ?>            <li style="float: right;cursor: pointer;">
                <a title="发表文章" onclick="login()">发表文章</a></li>
            <li style="float: right;cursor: pointer;">
                <a title="我要提问" onclick="login()">我要提问</a></li>
