...
<? if(is_array($attentionlist)) { foreach($attentionlist as $attention) { ?>
                     
                     
                      <div class="col-sm-3 mar-t-1">
                    <a title="<?=$attention['username']?>" class="block" target="_blank" href="<?=SITE_URL?>u-<?=$attention['uid']?>.html"><img width="80" height="80"  class="img-rounded" src="<?=$attention['avatar']?>" onmouseout="pop_user_out();" onmouseover="pop_user_on(this, '<?=$attention['uid']?>', 'image_active');" \></a><a class="name block" title="<?=$follower['follower']?>" target="_blank" href="<?=SITE_URL?>u-<?=$attention['uid']?>.html"><?=$attention['username']?></a>
                    </div>
