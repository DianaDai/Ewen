...
<? include template('user_title'); ?>
    ...
    <div class="row">
                 ...
                 <form name="favoriteForm" method="post" action="<?=SITE_URL?>favorite/delete.html" onsubmit="return check_form();">      
                    
                    <? if($favoritelist) { ?>   
    <ul class="main_con_qiangda_list clearfix">
  
            
<? if(is_array($favoritelist)) { foreach($favoritelist as $question) { ?>
                
        <li>
            <a target="_blank" href="<?=SITE_URL?>q/<?=$question['qid']?>.html" title="<?=$question['title']?>">
                <div class="left"><?=$question['answers']?><br>回答</div>
                <div class="uright">
                    <p class="otw text-nowrap ">  <input name="id[]" type='checkbox' value="<?=$question['id']?>" />&nbsp; 
                   
                    <? if($question['adopttime']>0) { ?>                     <i class="icon icon-check-sign text-danger mar-r-03"></i>
                    
                    <? } else { ?>  <i class="icon icon-question-sign text-danger mar-r-03"></i><? } ?>                  
