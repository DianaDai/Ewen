<? if(!defined('IN_ASK2')) exit('Access Denied'); include template('header'); $adlist = $this->fromcache("adlist"); ?><div class="container  mar-b-05 ">
<div class="row">
...
<? if(is_array($sublist)) { foreach($sublist as $index => $sub) { ?>
                <? if($sub['id']==$cid) { ?>                <li>
                <a  title="<?=$sub['name']?>"  href="<?=SITE_URL?>c-<?=$sub['id']?>.html">
                 <span class="text-danger bold"> <? echo cutstr($sub['name'],20,''); ?></span>

                <span class="label  label-danger"><?=$sub['questions']?></span>
                
                </a>
               </li>
                <? } else { ?>                <li><a title="<?=$sub['name']?>"  href="<?=SITE_URL?>c-<?=$sub['id']?>.html">
                <? echo cutstr($sub['name'],20,''); ?> <span class="label  "><?=$sub['questions']?></span></a>
                                    ...
                                    <div class="tit-full">
                                        <div class="wrap">
                                            ...
                                            <span class="gold">                
                                                ...
                                                <? if($question['shangjin']!=0) { ?>                      <span class="icon_hot">悬赏<?=$question['shangjin']?>元</span>
                    <? } ?>                                              
                                            </span>
...
<? } } ?>
   ...
   <? if($expertlist) { ?>      <div class="side-box">
...
<? } } ?>
 ...
 <!--广告位2-->
        ...
        <? } ?>         <? $attentionlist=$this->fromcache('attentionlist'); ?>         <? if($attentionlist) { ?>            <div class="side-box">
          <!-- 关注问题排行榜 -->
...
<? } } ?>
              ...
              <? } ?>              
               <? if($topiclist) { ?>                 <div class="side-box">
            <!-- 相关文章 -->
